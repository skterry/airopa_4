...
%\footnote[1]{\url{http://www.astro.ucla.edu/~ghezgroup/gc/index.shtml}} \textbf{(footnote to GCG webpage or something?)}.
...
\indent The left panel of Figure \ref{fig:ob150029-targ-res} shows a zoom on the microlensing target, the middle and right panels show the residual images after fitting and subtracting the static PSF model and variable PSF model, respectively. The residuals looks very similar, as expected for the brightest star in the field that is effectively located at the on-axis position.
...
\begin{figure}[!h]
  ...
  \caption{\footnotesize Ratio of astrometric errors and FVU values for the datasets; OB150029 (\textit{left column}), M53 PA$=$0 (\textit{middle column}), and M53 PA$=$90 (\textit{right column}). Values larger than one indicate the variable-PSF mode resulted in smaller astrometric/photometric residuals and smaller FVU, respectively.} \label{fig:ob-m53-astromfvu-ratios}
\end{figure}
...
For the M53 datasets, the astrometric precision is very similar between the two PSF modes for both PAs as can be seen in Figures \ref{fig:m53_PA_compare} and \ref{fig:ob-m53-astromfvu-ratios}. For nearly all of the stars the FVU between single and variable modes differ by ${\sim}$0.04 at most. The astrometric residuals after transforming the PA$=$0 star lists to the PA$=$90 reference frame have a dispersion of ${\sim}3$ mas, as can be seen in the lower-right panel of Figure \ref{fig:m53_PA_compare}. The distribution of astrometric residuals for all cross-matched stars between the two PAs is also very similar (lower-left panel of Figure \ref{fig:m53_PA_compare}).
